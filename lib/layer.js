 ...
 * 转换为combo链接
 *
 * @param  {Array} arr 原始链接列表
 * @return {Array}     combo链接列表
 */
var convert2comboUrls = function (arr, combo) {
    var result = {};

    arr.map(function (url) {
        return url.replace(/(https?:)?(\/\/.*?)?(\/.*)/, function (match, protocal, domain, path) {
            protocal = protocal || '';
            domain = domain || '';
            var key = protocal + domain;
    ...
    for (var key in result) {
        result[key].reduce(function(paths, path, index) {
            var isEnd = false;

            if ((paths.join(',').length + path.length) >= URL_LIMITE) {
                isEnd = true;
            }
            else if (index == result[key].length - 1) {
                paths.push(path);
                isEnd = true;
            }
            else {
            ...
            if (isEnd) {
                var comboUrl = (key || '') + ((typeof combo == 'string') ? combo : '') + '?' + paths.join(',');
                comboUrls.push(comboUrl);
                return [];
            }
...
var createHanlder = module.exports = function (res, options) {
    ...
    return {
        ...
        filterJs: function (content) {
            ...
            data.js = options.combo ? convert2comboUrls(data.js, options.combo) : data.js;
        ...
        filterCss: function (content) {
            ...
            data.css = options.combo ? convert2comboUrls(data.css, options.combo) : data.css;
